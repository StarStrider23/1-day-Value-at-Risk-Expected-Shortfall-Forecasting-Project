...
def var_es_comparison(data, 
    ...
    if backtesting is True:
        ...
        violations = test.compute_violations(data_backtesting, var_list)
        results["Number of violations"] = int(sum(violations))
        results["Sample size"] = len(var_list)

    if backtesting is True and tests is True:
        # Kupiec test
        results["Kupiec test"] = test.kupiec_test(violations)

        # Christoffersen test
        results["Christoffersen test"] = test.christofferesen_test(violations)
